...
class Lexer {
    constructor(options) {
        ...
        if (this.options.indent) this.options.indent = new Indent(options.indent);
    }
...
// ## _detectIndentUnit
// Detects indentation unit from a source String.
//
// Returns an Indent.
//
// For now returns either tab or space based on first indent match
//
// TODO: get all matches, gcd matches
function _detectIndentUnit (source) {
    let search = /(^\t+|^ +)\S+/m.exec(source);
    if (search !== null) {
        return new Indent(search[1][0]);
    } else return null
}
